...
namespace X_wing.Core
{
    public class BelongsToMany
    {
        ...
        private Dictionary<string,Dictionary<ModelCore,Dictionary<string,string>>> m_results = new Dictionary<string, Dictionary<ModelCore, Dictionary<string, string>>>();

        public Dictionary<string, Dictionary<ModelCore, Dictionary<string, string>>> results
        {
        ...
        /// </summary>
        private Dictionary<string,string> m_pivot = new Dictionary<string, string>();
        ...
        public BelongsToMany(ModelCore ModelBase,Type TypeModelALier, string table, string foreignKey, string otherKey, string relationName = "")
        {
            ...
            Dictionary<ModelCore, Dictionary<string, string>> resultats = new Dictionary<ModelCore, Dictionary<string, string>>();
            ...
            foreach (MyDB.IRecord record in App.recuperation(table, foreignKey, ModelBase.id))
            {
                ...
                resultats.Add((ModelCore)ModelALier,new Dictionary<string, string>());

                foreach (KeyValuePair<string, object> elem in record)
                {
                    if (elem.Key != otherKey && elem.Key != foreignKey)
                    {
                        if (!resultats[(ModelCore)ModelALier].ContainsKey(elem.Key))
                        {
                            resultats[(ModelCore)ModelALier].Add(elem.Key, elem.Value.ToString());
                        }
                        else
                        {
                            resultats[(ModelCore)ModelALier][elem.Key] = elem.Value.ToString();
                        }
                    }
                }
